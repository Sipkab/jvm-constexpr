...
/**
 * Tests that the CLI main can be run on mixed type of inputs, and that the output JAR is properly written.
 */
@SakerTest
public class MixInputTypeOutputJarCliTest extends CliTestCase {

	@Override
	protected void runTestImpl(Map<String, String> parameters) throws Throwable {
		Path testbasedir = testCaseDirectory;
		Path outputjarpath = testbasedir.resolve("output.jar");
		...
		args.add("-output");
		args.add(outputjarpath.toString());

		System.out.println("Args: " + args);

		CliMain.main(args.toArray(new String[0]));

		Map<String, ClassNode> nodes = TestUtils.jarToClassNodes(outputjarpath);
		//the -classpath shouldn't be written to the output jar
